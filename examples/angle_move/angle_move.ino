...
void setup()
{
  ...
  motor.setSpeed(40); //move fordward at 80 RPM's
  motor.setAngle(360); //move 360° until auto-stop triggers
  motor.setRPM_PID_KP(1.70); //set gains for motor RPM PID control
  motor.setRPM_PID_KD(22.11);
  motor.setRPM_PID_KI(0.08);
  motor.setATS_PID_KP(2.48); //set gains for auto-stop PID control
...
void loop()
{
  do
  {
    Serial.print("Total Moved Angle: "); Serial.println(motor.getAngle());
